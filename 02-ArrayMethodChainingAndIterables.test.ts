...
describe('chaining array methods', () => {
  ...
  it('should transform objects and sort by a specified property', () => {
    ...
    const sortedProducts = products.map((product) => ({priceWithTax: (product.price * 1.15).toFixed(2)})).sort
    ((a, b) => a.priceWithTax > b.priceWithTax ? 1: -1); // products.map().sort();
    ...
    // Initial test set to fail, correct answer should have all products with tax and sorted
  });
...
describe('iterables', () => {
  ...
  it('should demonstrate that objects are not iterable', () => {
    ...
    expect(keyValuePairs).toEqual([['a', 1], ['b', 2], ['c', 3]]); // Initial test set to fail, correct answer should be [['a', 1], ['b', 2], ['c', 3]]
  });
